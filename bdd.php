...
function getEquipe($BDD, $ID){
    ...
    if($res) {
 	while(mysqli_stmt_fetch($stmt)){
 	    array_push($equipe, array('pos' => $pos, 'ID' => $IDPkm, 'nom' => $nom, 'pv' =>$PV));
	}
    }
    return $equipe;
}
...
function displayEquipe($eq) {
    ...
    echo "<ul>";
    for ($i=1; $i < $taille; $i++) { 
 	echo "<li> IdPkm : ".$eq[$i]['ID']."<br/>position : ".$eq[$i]['pos']."<br/>nom : ".$eq[$i]['nom']."<br/>pv : ".$eq[$i]['pv']."pv</br><input type='button' onclick='send(1, ".$i.")' value='PC'/>";
        if ($eq[$i]['pos'] != 1){
...
function getPkmPc($BDD, $ID){
    $stmt = mysqli_prepare($BDD,
                           "SELECT PCPkm, nom, PV 
                            from PC, Pokedex, Pokemon 
                            where IDPkm=PCPkm and IDPC=? and IDPkd_=IDPkd 
                            order by IDPC ASC");
    mysqli_stmt_bind_param($stmt, 'i', $ID);
    mysqli_execute($stmt);
    ...
    if($res) {
 	while(mysqli_stmt_fetch($stmt)){
 	    array_push($pc, array('ID' => $IDPkm, 'nom' => $nom, 'pv' =>$PV));
	}
    }
    return $pc;
}
...
function displayPkmPC($pc) {
    ...
    echo "<ul>";
    for ($i=0; $i < $taille; $i++) { 
 	echo "<li><input type='button' onclick='send(3, ".$i.")' value='idPkm : ".$pc[$i]['ID'].", nom : ".$pc[$i]['nom'].", pv : ".$pc[$i]['pv']." pv'/></li>	";
 	echo "<br/>";
    }
    echo "</ul>";
